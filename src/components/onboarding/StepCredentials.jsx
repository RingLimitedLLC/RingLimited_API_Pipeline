...
import { KeyRound, ArrowRight, ArrowLeft, Eye, EyeOff, Info, ArrowDownToLine } from "lucide-react";
...
export default function StepCredentials({ form, update, onNext, onBack }) {
  ...
  const requiredFields = credFields.filter((f) => f.required);

  const updateField = (key, value) =>
    update({ connection_type_fields: { ...form.connection_type_fields, [key]: value } });

  const canProceed = isInbound ||
    requiredFields.every((f) => (form.connection_type_fields?.[f.key] || "").trim());

  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3 mb-2">
        <div className="h-10 w-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: "#afd741" }}>
          {isInbound ? <ArrowDownToLine className="h-5 w-5 text-white" /> : <KeyRound className="h-5 w-5 text-white" />}
        </div>
        <div>
          <h2 className="text-lg font-semibold text-slate-900">
            {isInbound ? "Inbound Connection" : "API Credentials"}
          </h2>
          <p className="text-sm text-slate-500">{ct?.label}</p>
        </div>
      </div>

      {isInbound ? (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800 space-y-1">
          <p className="font-medium">This is an inbound connection</p>
          <p className="text-blue-700">The client pushes data to a generated webhook URL. No outbound API credentials are needed. A webhook endpoint will be available after the connection is created.</p>
        </div>
      ) : (
        hasSecrets && (
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 flex gap-2 text-xs text-slate-600">
            <Info className="h-4 w-4 text-slate-400 shrink-0 mt-0.5" />
            Credentials are stored encrypted in 1Password and never shown in the UI after saving.
          </div>
        )
      )}
